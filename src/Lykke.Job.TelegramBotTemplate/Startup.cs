...
using Lykke.Job.TelegramBotTemplate.Modules;
using Lykke.Job.TelegramBotTemplate.Settings;
using Lykke.Logs;
...
namespace Lykke.Job.TelegramBotTemplate
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            try
            {
                ...
                {
                    options.DefaultLykkeConfiguration("v1", "TelegramBotTemplate API");
                });
                ...
                builder.RegisterModule(new JobModule(appSettings.CurrentValue.TelegramBotTemplateJob, appSettings.Nested(x => x.TelegramBotTemplateJob), Log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            try
            {
                ...
                app.UseLykkeMiddleware("TelegramBotTemplate", ex => new ErrorResponse {ErrorMessage = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, IReloadingManager<AppSettings> settings)
        {
            ...
            var dbLogConnectionStringManager = settings.Nested(x => x.TelegramBotTemplateJob.Db.LogsConnString);
            ...
            var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                AzureTableStorage<LogEntity>.Create(dbLogConnectionStringManager, "TelegramBotTemplateLog", consoleLogger),
                consoleLogger);
